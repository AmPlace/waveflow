...
import httpx
from streamget import FlexTVLiveStream

from . import ADAPTER_SUCCESS_TTL_SECONDS, AdapterRequest, AdapterResolveError


async def resolve_flextv(request: AdapterRequest, client: httpx.AsyncClient) -> dict[str, Any]:
    room_id = request.resource_id.strip("/")
    if not room_id:
        raise AdapterResolveError("invalid_flextv_room_id", "FlexTV 房间号不能为空")

    url = f"https://www.ttinglive.com/channels/{room_id}/live"

    try:
        live = FlexTVLiveStream(cookies="")
