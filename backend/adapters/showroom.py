...
import httpx
from streamget import ShowRoomLiveStream

from . import ADAPTER_SUCCESS_TTL_SECONDS, AdapterRequest, AdapterResolveError


async def resolve_showroom(request: AdapterRequest, client: httpx.AsyncClient) -> dict[str, Any]:
    room_id = request.resource_id.strip("/")
    if not room_id:
        raise AdapterResolveError("invalid_showroom_room_id", "SHOWROOM 房间号不能为空")

    url = f"https://www.showroom-live.com/room/profile?room_id={room_id}"

    try:
        live = ShowRoomLiveStream(cookies="")
