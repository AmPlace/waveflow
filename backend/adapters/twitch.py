...
async def resolve_twitch(request: AdapterRequest, client: httpx.AsyncClient) -> dict[str, Any]:
    room_id = request.resource_id.strip("/")
    if not room_id:
        raise AdapterResolveError("invalid_twitch_room_id", "Twitch 房间号不能为空")

    url = f"https://www.twitch.tv/{room_id}"

    try:
        live = TwitchLiveStream(cookies="")
