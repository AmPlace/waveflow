...
import httpx
from streamget import HuamaoLiveStream

from . import ADAPTER_SUCCESS_TTL_SECONDS, AdapterRequest, AdapterResolveError


async def resolve_huamao(request: AdapterRequest, client: httpx.AsyncClient) -> dict[str, Any]:
    room_id = request.resource_id.strip("/")
    if not room_id:
        raise AdapterResolveError("invalid_huamao_room_id", "花猫 房间号不能为空")

    url = f"https://www.huamao.com/{room_id}"

    try:
        live = HuamaoLiveStream(cookies="")
