...
import httpx
from streamget import HuajiaoLiveStream

from . import ADAPTER_SUCCESS_TTL_SECONDS, AdapterRequest, AdapterResolveError


async def resolve_huajiao(request: AdapterRequest, client: httpx.AsyncClient) -> dict[str, Any]:
    room_id = request.resource_id.strip("/")
    if not room_id:
        raise AdapterResolveError("invalid_huajiao_room_id", "花椒 房间号不能为空")

    url = f"https://www.huajiao.com/l/{room_id}"

    try:
        live = HuajiaoLiveStream(cookies="")
