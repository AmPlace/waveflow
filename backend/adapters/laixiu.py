...
import httpx
from streamget import LaixiuLiveStream

from . import ADAPTER_SUCCESS_TTL_SECONDS, AdapterRequest, AdapterResolveError


async def resolve_laixiu(request: AdapterRequest, client: httpx.AsyncClient) -> dict[str, Any]:
    room_id = request.resource_id.strip("/")
    if not room_id:
        raise AdapterResolveError("invalid_laixiu_room_id", "来秀 房间号不能为空")

    url = f"https://www.laixiu.com/{room_id}"

    try:
        live = LaixiuLiveStream(cookies="")
