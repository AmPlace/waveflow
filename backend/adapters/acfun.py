...
import httpx
from streamget import AcfunLiveStream

from . import ADAPTER_SUCCESS_TTL_SECONDS, AdapterRequest, AdapterResolveError


async def resolve_acfun(request: AdapterRequest, client: httpx.AsyncClient) -> dict[str, Any]:
    room_id = request.resource_id.strip("/")
    if not room_id:
        raise AdapterResolveError("invalid_acfun_room_id", "AcFun 房间号不能为空")

    url = f"https://live.acfun.cn/{room_id}"

    try:
        live = AcfunLiveStream(cookies="")
