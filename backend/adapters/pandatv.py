...
import httpx
from streamget import PandaLiveStream

from . import ADAPTER_SUCCESS_TTL_SECONDS, AdapterRequest, AdapterResolveError


async def resolve_pandatv(request: AdapterRequest, client: httpx.AsyncClient) -> dict[str, Any]:
    room_id = request.resource_id.strip("/")
    if not room_id:
        raise AdapterResolveError("invalid_pandatv_room_id", "PandaTV 房间号不能为空")

    url = f"https://www.pandalive.co.kr/{room_id}"

    try:
        live = PandaLiveStream(cookies="")
