...
import httpx
from streamget import LianJieLiveStream

from . import ADAPTER_SUCCESS_TTL_SECONDS, AdapterRequest, AdapterResolveError


async def resolve_lianjie(request: AdapterRequest, client: httpx.AsyncClient) -> dict[str, Any]:
    room_id = request.resource_id.strip("/")
    if not room_id:
        raise AdapterResolveError("invalid_lianjie_room_id", "来连结 房间号不能为空")

    url = f"https://www.lailianjie.com/{room_id}"

    try:
        live = LianJieLiveStream(cookies="")
