...
import httpx
from streamget import KugouLiveStream

from . import ADAPTER_SUCCESS_TTL_SECONDS, AdapterRequest, AdapterResolveError


async def resolve_kugou(request: AdapterRequest, client: httpx.AsyncClient) -> dict[str, Any]:
    room_id = request.resource_id.strip("/")
    if not room_id:
        raise AdapterResolveError("invalid_kugou_room_id", "酷狗 房间号不能为空")

    url = f"https://fanxing.kugou.com/{room_id}"

    try:
        live = KugouLiveStream(cookies="")
