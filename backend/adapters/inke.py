...
import httpx
from streamget import InkeLiveStream

from . import ADAPTER_SUCCESS_TTL_SECONDS, AdapterRequest, AdapterResolveError


async def resolve_inke(request: AdapterRequest, client: httpx.AsyncClient) -> dict[str, Any]:
    room_id = request.resource_id.strip("/")
    if not room_id:
        raise AdapterResolveError("invalid_inke_room_id", "映客 房间号不能为空")

    url = f"https://webapi.busi.inke.cn/web/live_share_pc?id={room_id}"

    try:
        live = InkeLiveStream(cookies="")
