...
import httpx
from streamget import MaoerLiveStream

from . import ADAPTER_SUCCESS_TTL_SECONDS, AdapterRequest, AdapterResolveError


async def resolve_maoer(request: AdapterRequest, client: httpx.AsyncClient) -> dict[str, Any]:
    room_id = request.resource_id.strip("/")
    if not room_id:
        raise AdapterResolveError("invalid_maoer_room_id", "猫耳 房间号不能为空")

    url = f"https://fm.missevan.com/{room_id}"

    try:
        live = MaoerLiveStream(cookies="")
