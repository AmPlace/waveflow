...
import httpx
from streamget import JDLiveStream

from . import ADAPTER_SUCCESS_TTL_SECONDS, AdapterRequest, AdapterResolveError


async def resolve_jd(request: AdapterRequest, client: httpx.AsyncClient) -> dict[str, Any]:
    room_id = request.resource_id.strip("/")
    if not room_id:
        raise AdapterResolveError("invalid_jd_room_id", "京东 房间号不能为空")

    url = f"https://lives.jd.com/{room_id}"

    try:
        live = JDLiveStream(cookies="")
