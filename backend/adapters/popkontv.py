...
import httpx
from streamget import PopkonTVLiveStream

from . import ADAPTER_SUCCESS_TTL_SECONDS, AdapterRequest, AdapterResolveError


async def resolve_popkontv(request: AdapterRequest, client: httpx.AsyncClient) -> dict[str, Any]:
    room_id = request.resource_id.strip("/")
    if not room_id:
        raise AdapterResolveError("invalid_popkontv_room_id", "PopkonTV 房间号不能为空")

    url = f"https://www.popkontv.com/live/view?castId={room_id}"

    try:
        live = PopkonTVLiveStream(cookies="")
